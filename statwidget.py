        ...
        if index.isValid():
            if role == Qt.BackgroundRole:
                ...
                if self.data_frame.values[index.row()][0] != '--':
                    ...
                    return QBrush(QColor(rgb_color[0], rgb_color[1], 
                    rgb_color[2], transparency))
...
class StatsGui(QWidget, HubListener):
    ...
    def __init__(self, dc):
        ...
        dc.hub.subscribe(self, SubsetUpdateMessage, handler=self.messageReceived)  
    ...
    def myPressedEvent (self, currentQModelIndex):
        ...
        for index in range (0, len(newly_dropped)):
            ...
            is_subset = newly_dropped[index].parent().parent().parent().row() == 1 
            or (self.switch_mode.text() == 'Sort tree by subsets' and subset_i != -1)
            ...
            if is_subset:
                try:
                    ...
                    idx_d = np.where(self.data_frame['Dataset'] == 
                    self.dc[data_i].label)
    ...
    def runDataStats (self, data_i, comp_i):
        ...
        # Save the data in the cache 
            mean_val = self.dc[data_i].compute_statistic('mean', 
            self.dc[data_i].components[comp_i])
            median_val = self.dc[data_i].compute_statistic('median', 
            self.dc[data_i].components[comp_i])     
            min_val = self.dc[data_i].compute_statistic('minimum', 
            self.dc[data_i].components[comp_i])     
            max_val = self.dc[data_i].compute_statistic('maximum', 
            self.dc[data_i].components[comp_i])    
            sum_val = self.dc[data_i].compute_statistic('sum', 
            self.dc[data_i].components[comp_i])

            column_data = np.asarray([[subset_label], [data_label], [comp_label], 
            [mean_val], [median_val], [min_val], [max_val], [sum_val]]).transpose()
    ...
    def runSubsetStats (self, subset_i, data_i, comp_i):
        ...
        # Save in the cache
            ...
            component_data = self.dc[data_i].subsets[subset_i].components[comp_i]
            state = self.dc.subset_groups[subset_i].subset_state

            mean_val = self.dc[data_i].compute_statistic('mean', 
            component_data, subset_state=state)
            median_val = self.dc[data_i].compute_statistic('median', 
            component_data, subset_state=state)       
            min_val = self.dc[data_i].compute_statistic('minimum', 
            component_data, subset_state=state)       
    ...
    def sigchange(self, i):
        ...
        column_data = np.asarray([subset_labels, data_labels, comp_labels, 
        mean_vals, median_vals, min_vals, max_vals, sum_vals]).transpose()
    ...
    def visibleClicked(self):
        ...
        if len(new_rows) == 0:
            ...
            message_widget, 'Message', text, QMessageBox.Close, QMessageBox.Close)
    ...
    def allClicked(self):
        ...
        if num_to_add > 20:
            ...
            text = "Are you sure you want to add " + str(num_to_add) 
            + " rows to the table?"
            ...
            # Above text, Yes, and Cancel with Cancel default selected
            result = QMessageBox.warning(message_widget, 
            'Message', text, QMessageBox.Yes | QMessageBox.Cancel, QMessageBox.Cancel)
    ...
    def switchMode(self):
        # If the user clicks to sort by components:
        # Change the text to "sort by subsets" and sort tree by components
        if self.switch_mode.text() == 'Sort tree by components':
            self.sortByComponents()
            self.switch_mode.setText('Sort tree by subsets')
        # Otherwise the user wants to sort by subsets:
        # Change text to "sort by components" and sort tree by subsets
        else:
            self.sortBySubsets()
            self.switch_mode.setText('Sort tree by components')
    ...
    def generateSubsetView(self):
        ...
        # Populate the tree
        # Make all the datasets be parents
        # Make it so they are not selectable
        ...
        for i in range(0, len(self.dc)):
            ...
            for j in range(0,len(self.dc[i].components)):
                ...
                key = self.dc[i].label 
                + self.dc[i].components[j].label 
                + "All data-" + self.dc[i].label
        ...
        for j in range(0, len(self.dc.subset_groups)):
            ...
            for i in range(0, len(self.dc)):
                ...
                try:
                    self.dc[i].compute_statistic('mean', 
                ...
                for k in range(0, len(self.dc[i].components)):
                    ...
                    key = self.dc[i].label + self.dc[i].components[k].label 
                    + self.dc[i].subsets[j].label
                    self.subset_dict[key] = child.index()
...
#                             print("Glue has raised an Incompatible Attribute error on this component. Let's do this instead.")
        ...
        for i in range(0, parent_data.rowCount()):
            for j in range(0, parent_data.child(i).rowCount()):
                key = "All data (" + parent_data.child(i).text() + 
                ")" + parent_data.child(i).child(j).text()
        ...
        for i in range(0, parent_subset.rowCount()):
            for j in range(0, parent_subset.child(i).rowCount()):
                for k in range(0, parent_subset.child(i).child(j).rowCount()):
                    ...
                    self.subset_dict[key] = 
                    parent_subset.child(i).child(j).child(k).index()
        ...
        for i in range(0,len(dc)):
            ...
            for k in range(0, len(self.dc[i].components)):
                ...
                for j in range(0, len(self.dc.subset_groups)):
                    ...
                    try:
                        self.dc[i].compute_statistic('mean', self.dc[i].subsets[j].components[k], 
                        subset_state=self.dc[i].subsets[j].subset_state)
            ...
            # Fill out the dict now that the indices 
            # are connected to the QStandardItemModel
            for i in range(0, grandparent.rowCount()):
                for j in range(0, grandparent.child(i).rowCount()):
                    if grandparent.child(i).child(j).row() == 0:
                        key = grandparent.child(i).child(j).text() 
                        + grandparent.child(i).text()
                        self.component_dict[key] = 
                        grandparent.child(i).child(j).index()
                    else:
                        key = grandparent.child(i).child(j).text() + " (" + grandparent.text() 
                        + ")" + grandparent.child(i).text()
                        self.component_dict[key] = 
                        grandparent.child(i).child(j).index()
    ...
    def notation(self):
        ...
        for i in range(0, len(self.data_frame)):
            # Traverse through the dataframe
            # Get the names of the component, dataset, and subset
            component = self.data_frame['Component'][i]
            dataset = self.data_frame['Dataset'][i]
            subset = self.data_frame['Subset'][i]
            ...
            mean_vals.append(string % Decimal(self.data_accurate['Mean']
            [idx2]))
            median_vals.append(string % Decimal(self.data_accurate['Median']
            [idx2]))
            min_vals.append(string % Decimal(self.data_accurate['Minimum'][idx2]))
            max_vals.append(string % Decimal(self.data_accurate['Maximum']
            [idx2]))
            sum_vals.append(string % Decimal(self.data_accurate['Sum']
            [idx2]))

        # Build the column_data and update the data_frame
        column_data = np.asarray([subset_labels, data_labels, comp_labels,
                    mean_vals, median_vals, min_vals, max_vals,
                    sum_vals]).transpose()
        self.data_frame = pd.DataFrame(column_data, columns=self.headings)
        model = pandasModel(self.data_frame, self.dc)
        self.table.setModel(model)
...
# subset_group1.label = "First DE Label"
# subset_group1.style.color = '#00FFFF'
# subset_group1.label = 'Second DE Label'
# subset_group1.style.color = '#FFFF00'
